...
def ricevi_comandi(sock_service, addr_client):
    with sock_service as sock_client:
        try:
            ricevuto = sock_client.recv(DIM_BUFFER).decode()
            if not ricevuto:
                print(f"Client {addr_client} disconnesso senza inviare dati.")
                return

            dati = json.loads(ricevuto)
            ...
            print("Errore: I dati ricevuti non sono in un formato JSON valido.")
        except Exception as e:
            print(f"Errore inatteso con il client {addr_client}: {e}")
...
def avvia_server(indirizzo, porta):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock_server:
        ...
        # SPOSTATO DENTRO IL BLOCCO 'WITH':
